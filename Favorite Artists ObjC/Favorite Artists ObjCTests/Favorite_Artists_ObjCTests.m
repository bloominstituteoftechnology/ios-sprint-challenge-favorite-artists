...
- (NSData *)getBadData {
	NSBundle *bundle = [NSBundle bundleForClass:[self class]];
	NSData *data = loadFile(@"BadArtistResults.json", bundle);
...
- (void)testArtistDecode {
	...
	REPCoreDataStack *sharedStack = [REPCoreDataStack sharedInstance];
	NSManagedObjectContext* context = sharedStack.mainContext;
...
- (void)testBadJSON {
	...
	XCTAssertEqualObjects(@"unknown", artist.name);
	XCTAssertEqualObjects(@"unknown", artist.genre);
	XCTAssertEqual(0, artist.yearFormed);
	XCTAssertEqualObjects(@"unknown", artist.mood);
	XCTAssertEqualObjects(@"unknown", artist.bio);
