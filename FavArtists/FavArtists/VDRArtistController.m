...
@interface VDRArtistController ()
@property NSMutableArray *internalArtistsArray;
@end

@implementation VDRArtistController
...
- (VDRArtist *)fetchSavedArtist:(VDRArtist *)artist {
    ...
    VDRArtist *savedArtist = [[VDRArtist alloc] initWithDictionary:dictionary];
    return savedArtist;
}
...
- (NSMutableArray *)fetchAllSavedArtists {
    ...
    for (NSString *artist in filePaths) {
        ...
        if (artistData != nil) {
            ...
            VDRArtist *artist = [[VDRArtist alloc] initWithDictionary:artistDictionary];
            [self.internalArtistsArray addObject:artist];
        } else {
...
- (void)searchForArtist:(NSString *)name completionBlock:(VDRArtistSearchCompletionBlock)completionBlock {
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        VDRArtist *artist = [[VDRArtist alloc] initWithDictionary:dictionary[0]];
